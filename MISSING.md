...
The offline scalper demo and regression suite cover the positive, duplicate,
and unverified paths. The live Hedera run remains the integration demo rather
than a hard-coded claim in this document. This is the honest gap list and trust
model, written for the team and reusable for the prize submission.
...
- `src/server/world-gateway.ts` — consumes AgentKit's anti-sybil primitive
  ...
  in an Ed25519-signed `AuctionPass` bound to one leaf wallet and one auction.
...
- **Proof of address control**: settlement accepts no bare AgentBook address.
  The browser signs a short-lived, single-use challenge bound to the plan ID;
  the coordinator consumes it before the canonical AgentBook lookup.
- **Public authorization evidence**: the gateway issuer key is pinned in
  `LISTED`. Successful credentials are rechecked by seller policy and recorded
  as `AUTHORIZED` with the bidder, auction-scoped nullifier, expiry, quota, and
  Ed25519 signature before the seller signs the atomic swap.
- **Live sybil demo**: `worldDemo: "scalper"` on the jobs API collapses the
  human-backed rival personas into one underlying human while the deliberately
  unverified rival remains unverified; the gateway then admits exactly one of
  the shared human's agents per protected item and blocks the rest at
  settlement. The job exposes
  `world: { passesIssued, sybilRejections, blocked[] }` for the UI.
  `npm run demo:scalper` remains as the fast offline version.
- Tests pinning the trust model: sybil collapse, cross-auction unlinkability,
  bot rejection, pass tamper-evidence, address-impersonation rejection,
  challenge replay rejection, explicit mock negatives, fail-closed market
  configuration, and authenticated HCS authorization evidence.
- **In-product registration at `/world`**: the AgentKit CLI flow in the
  ...
  canonical AgentBook on World Chain (`job.world.userIdentityProved: true`).
  Rival personas are explicit fixtures: Bruno and Emma are mock human-backed;
  Chiara is deliberately unverified. A negative mock result cannot fall
  through to the real AgentBook.
...
2. **Remaining UI details.** Badges and the front-page status shipped;
...
3. **Bid-level enforcement (optional hardening).** Precisely: unverified
...
4. **Gateway quota persistence across jobs.** Quotas live for the duration
...
| **Credential gateway** | humanId ↔ leaf-wallet links, briefly, at enrollment | Spend money or sign as a buyer; it can lie about its private lookup because it is trusted | Auction-scoped zero-knowledge credentials |

What each party must trust about the gateway:

- **Sellers** trust it to perform the private AgentBook lookup and enforce the
  per-human quota honestly. They independently verify the Ed25519 pass and see
  distinct nullifiers per bidder in `AUTHORIZED`; duplicate allocation for one
  nullifier is detectable. HCS cannot prove that the hidden AgentBook lookup
  happened, so fabricated distinct credentials remain inside the declared
...
- **Buyers** trust it not to log the humanId ↔ wallet mapping. This is a
  ...
  deanonymize link patterns or issue a dishonest market credential, but it
  cannot alter the canonical AgentBook, sign as the buyer, or take funds.
...
> privacy and correctness trust point whose signed authorization decisions
> are public; it can neither alter World identity nor touch funds.
